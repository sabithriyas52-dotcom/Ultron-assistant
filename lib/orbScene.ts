...
export interface OrbSceneApi {
  ...
  zoomOut(): void;
  resetView(): void;
  /** 0 = idle glow, 1 = fully "active" (listening/speaking) glow. Eases smoothly. */
...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  function animate() {
    ...
    // Bloom pulse (baseline breathing + assistant activity boost)
    activity += (activityTarget - activity) * 0.08;
    const activityPulse = activity * (1.1 + Math.sin(t * 6) * 0.35);
    bloom.strength = 1.6 + Math.sin(t * 0.8) * 0.3 + activityPulse;
